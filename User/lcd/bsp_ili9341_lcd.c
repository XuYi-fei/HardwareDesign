...
//extern unsigned char (*letters)[32] = letters1;
volatile uint16_t current_col = 0;
volatile uint8_t mode = 0;
volatile uint8_t col_num = 0;
void Chinese_Show(unsigned char(*letters)[32], int size1){
	...
	switch(mode){
		case 0:{
			...
			for(i = 0; i < 240; i++){
				...
				temp = letters[num_char][char_col*2];
				...
				temp = letters[num_char][char_col*2+1];
				for(t1=8; t1 < 16; t1++){
					if(temp & 0x80) ILI9341_SetPointPixel(i,LCD_Y_LENGTH/2+t1);
					temp <<= 1;
				}
				current_col = (current_col + 1) % (size1);
			}
			current_col = (last + 1) % (size1);
			break;
		}
		case 1:{
			volatile uint8_t num_char = 0, char_col = 0;
			...
			current_col = (last + size1 - 1 ) % (size1);
			break;
		}
		case 2:{
			volatile uint16_t tmp_col;
			volatile uint8_t num_char = 0, char_col = 0;
			volatile uint16_t last = current_col;
			...
			for(i = 0; i < 240; i++){
				...
				for(t1 = col_num ; t1 < 16; t1++){
					if(temp & 0x80) ILI9341_SetPointPixel(i,LCD_Y_LENGTH/2+point_row);
					point_row ++;
					temp <<= 1;
				}
				
  			tmp_col = (current_col + 240) % size1;
				num_char = tmp_col / 16;
				char_col = tmp_col % 16;
				temp = letters[num_char][char_col*2];
				for(t1 = 0; t1 < 8 && point_row < 16; t1 ++){
				 	if(temp & 0x80) ILI9341_SetPointPixel(i,LCD_Y_LENGTH/2+point_row);
					point_row ++;
				 	temp <<= 1;
				}
			...
			if(col_num == 15){
				col_num = 0;
				current_col = (last + LCD_X_LENGTH) % (size1);
			} else{
				col_num ++;
				current_col = last;
			}
